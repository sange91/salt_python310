...
def eni_present(
    ...
    if allocate_eip:
        if "allocationId" not in r["result"]:
            ...
            ret["comment"] = " ".join(
                [ret["comment"], "An EIP is already allocated/assocaited to the ENI"]
            )
    if arecords:
    ...
    **kwargs
    ...
    while True:
        ...
        if time() - starttime > wait_timeout_seconds:
            if images:
                ret["comment"] = "AMI still in state {state} after timeout".format(
...
def instance_present(
        ...
        for k, v in attributes.items():
            ...
            if curr.get(k) == v:
                ...
                if __opts__["test"]:
                    ...
                    continue
                try:
                    r = __salt__["boto_ec2.set_attribute"](
                        attribute=k,
                        attribute_value=v,
                        instance_id=instance_id,
                        region=region,
    ...
    if tags and instance_id is not None:
        tags = dict(tags)
        curr_tags = dict(
            __salt__["boto_ec2.get_all_tags"](
...
def volume_present(
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    if __salt__["boto_ec2.attach_volume"](
        volume_id=volume_id, instance_id=instance_id, device=device, **args
    ):
        ret["comment"] = " ".join(
            [
                ret["comment"],
