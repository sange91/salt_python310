...
def present(
    ...
    try:
        record = __salt__["boto_route53.get_record"](
            name,
            zone,
            record_type,
            False,
...
def hosted_zone_present(
    ...
    if not deets:
        ...
        if (
            salt.utils.json.loads(deets["HostedZone"]["Config"]["PrivateZone"])
            != private_zone
        ):
            create = True
        else:
        ...
        if not create:
            ...
            # settings...
            log.info(
                "A Hosted Zone with name %s already exists, but with "
...
def hosted_zone_absent(
    name, domain_name=None, region=None, key=None, keyid=None, profile=None
):
    """
