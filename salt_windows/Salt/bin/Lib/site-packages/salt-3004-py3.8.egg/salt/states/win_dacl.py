...
def present(name, objectType, user, permission, acetype, propagation):
    ...
    tRet = __salt__["win_dacl.check_ace"](
        name, objectType, user, permission, acetype, propagation, True
    )
    if tRet["result"]:
        if not tRet["Exists"]:
            if __opts__["test"]:
                ...
                return ret
            addRet = __salt__["win_dacl.add_ace"](
    ...
    if tRet["result"]:
        if tRet["Exists"]:
            if __opts__["test"]:
                ...
                return ret
            addRet = __salt__["win_dacl.rm_ace"](
                name, objectType, user, permission, acetype, propagation
            )
            if addRet["result"]:
                ret["result"] = True
        ...
        if not tRet["Inheritance"]:
            if __opts__["test"]:
                ...
                ret["changes"]["Existing ACLs"] = (
                    ...
                    if clear_existing_acl
                    else "Are set to be kept"
                )
                return ret
            eRet = __salt__["win_dacl.enable_inheritance"](
                name, objectType, clear_existing_acl
            )
            if eRet["result"]:
                ret["result"] = True
        ...
        if tRet["Inheritance"]:
            if __opts__["test"]:
                ...
                ret["changes"]["Inherited ACLs"] = (
                    "Are set to be kept"
                    if copy_inherited_acl
                    else "Are set to be removed"
                )
                return ret
            eRet = __salt__["win_dacl.disable_inheritance"](
                name, objectType, copy_inherited_acl
            )
