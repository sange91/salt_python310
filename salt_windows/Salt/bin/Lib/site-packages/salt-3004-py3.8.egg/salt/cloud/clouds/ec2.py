...
def query(
    ...
    while attempts < aws.AWS_MAX_RETRIES:
        ...
        ec2_api_version = provider.get("ec2_api_version", DEFAULT_EC2_API_VERSION)
    ...
    while True:
        ...
        if timeout < 0:
            raise SaltCloudExecutionTimeout(
                ...
                "00:{:02d}:{:02d}".format(int(duration // 60), int(duration % 60))
            )
        time.sleep(interval)
        timeout -= interval
...
def avail_sizes(call=None):
    ...
    sizes = {
        ...
        "General Purpose": {
            ...
            "t2.micro": {"id": "t2.micro", "cores": "1", "disk": "EBS", "ram": "1 GiB"},
            "t2.small": {"id": "t2.small", "cores": "1", "disk": "EBS", "ram": "2 GiB"},
            "t2.medium": {
                "id": "t2.medium",
                "cores": "2",
                "disk": "EBS",
            ...
            "t2.large": {"id": "t2.large", "cores": "2", "disk": "EBS", "ram": "8 GiB"},
            "t2.xlarge": {
                "id": "t2.xlarge",
                "cores": "4",
                "disk": "EBS",
...
def get_ssh_gateway_config(vm_):
    ...
    ssh_gateway = config.get_cloud_config_value(
        "ssh_gateway", vm_, __opts__, default=None, search_global=False
    )
    ...
    ssh_gateway_config["ssh_gateway_port"] = config.get_cloud_config_value(
        "ssh_gateway_port", vm_, __opts__, default=None, search_global=False
    )

    # ssh_gateway_username
    ssh_gateway_config["ssh_gateway_user"] = config.get_cloud_config_value(
        "ssh_gateway_username", vm_, __opts__, default=None, search_global=False
    )

    # ssh_gateway_private_key
    ssh_gateway_config["ssh_gateway_key"] = config.get_cloud_config_value(
        "ssh_gateway_private_key", vm_, __opts__, default=None, search_global=False
    )

    # ssh_gateway_password
    ssh_gateway_config["ssh_gateway_password"] = config.get_cloud_config_value(
        "ssh_gateway_password", vm_, __opts__, default=None, search_global=False
    )

    # ssh_gateway_command
    ssh_gateway_config["ssh_gateway_command"] = config.get_cloud_config_value(
        "ssh_gateway_command", vm_, __opts__, default=None, search_global=False
    )
...
def _create_eni_if_necessary(interface, vm_):
    ...
    return {"DeviceIndex": interface["DeviceIndex"], "NetworkInterfaceId": eni_id}
...
def _associate_eip_with_interface(eni_id, eip_id, private_ip=None, vm_=None):
    ...
    if not result[2].get("associationId"):
        raise SaltCloudException(
            ...
            "<{}> with network interface <{}>".format(eip_id, eni_id)
        )

    log.debug("Associated ElasticIP address %s with interface %s", eip_id, eni_id)
    ...
    for interface in interfaces:
        if "DeviceIndex" in interface:
            if interface["DeviceIndex"] in config_enis:
                log.error("Duplicate DeviceIndex in profile. Cannot update ENIs.")
                return None
...
def request_instance(vm_=None, call=None):
    ...
    vm_size = config.get_cloud_config_value("size", vm_, __opts__, search_global=False)
    ...
    if ex_iam_profile:
        try:
            ...
            raise SaltCloudConfigError("'iam_profile' should be a string value.")
    ...
    if network_interfaces:
        ...
        params.update(_param_from_config(spot_prefix + "NetworkInterface", eni_devices))
    ...
    if set_del_root_vol_on_destroy:
        ...
        rd_params = {"Action": "DescribeImages", "ImageId.1": image_id}
        try:
            rd_data = aws.query(
                rd_params,
                location=get_location(vm_),
        ...
        if not rd_data:
            err_msg = (
                "There was an error querying EC2 for the root device "
                "of image id {}. Empty response.".format(image_id)
            )
...
def query_instance(vm_=None, call=None):
    ...
    params = {"Action": "DescribeInstances", "InstanceId.1": instance_id}
    ...
    def __query_ip_address(params, url):  # pylint: disable=W0613
        ...
        if ssh_interface(vm_) == "public_ips":
            if "ipAddress" in data[0]["instancesSet"]["item"]:
                ...
                log.error("Public IP not detected.")

        if ssh_interface(vm_) == "private_ips":
            if "privateIpAddress" in data[0]["instancesSet"]["item"]:
                return data
            else:
                log.error("Private IP not detected.")
...
def wait_for_instance(
    ...
    if config.get_cloud_config_value("win_installer", vm_, __opts__):
        ...
        if win_passwd and win_passwd == "auto":
            ...
            while True:
                password_data = get_password_data(
                    name=vm_["name"],
                    kwargs={"key_file": vm_["private_key"]},
                    call="action",
                )
        ...
        if not use_winrm:
            ...
            if not salt.utils.cloud.validate_windows_cred(
                ip_address,
                username,
                win_passwd,
        ...
        # If using winrm
            ...
            if not salt.utils.cloud.wait_for_winrm(
                ip_address,
                winrm_port,
                username,
                win_passwd,
        ...
        for user in vm_["usernames"]:
            if salt.utils.cloud.wait_for_passwd(
                ...
                known_hosts_file=config.get_cloud_config_value(
                    "known_hosts_file", vm_, __opts__, default="/dev/null"
                ),
            ):
...
def create(vm_=None, call=None):
    ...
    if "spot_config" in vm_ and "tag" in vm_["spot_config"]:
        ...
        for value in vm_["spot_config"]["tag"].values():
            if not isinstance(value, str):
                raise SaltCloudConfigError(
                    "'tag' values must be strings. Try quoting the values. "
                    'e.g. "2013-09-19T20:09:46Z".'
    ...
    for key, value in __utils__["cloud.bootstrap"](vm_, __opts__).items():
        ret.setdefault(key, value)

    log.info("Created Cloud VM '%s'", vm_["name"])
    log.debug("'%s' VM creation details:\n%s", vm_["name"], pprint.pformat(instance))
    ...
    if ex_blockdevicetags:
        for _device, _map in ret["blockDeviceMapping"].items():
            bd_items = []
            ...
            for blockitem in bd_items:
                if (
                    blockitem["deviceName"] in ex_blockdevicetags
                    and "Name" not in ex_blockdevicetags[blockitem["deviceName"]]
                ):
...
def create_attach_volumes(name, kwargs, call=None, wait_to_finish=True):
    ...
    for volume in volumes:
        created = False
        volume_name = "{} on {}".format(volume["device"], name)

        volume_dict = {"volume_name": volume_name, "zone": kwargs["zone"]}
        ...
        if "volume_id" not in volume_dict:
            created_volume = create_volume(
                volume_dict, call="function", wait_to_finish=wait_to_finish
            )
            created = True
        ...
        if attach:
            msg = "{} attached to {} (aka {}) as device {}".format(
...
def stop(name, call=None):
    ...
    params = {"Action": "StopInstances", "InstanceId.1": instance_id}
    result = aws.query(
        params,
        location=get_location(),
        provider=get_provider(),
...
def set_tags(
    ...
    # a usable ID
    if instance_id is None:
        return {"Error": "A valid instance_id or resource_id was not specified."}

    params = {"Action": "CreateTags", "ResourceId.1": instance_id}
    ...
    while attempts < aws.AWS_MAX_RETRIES:
        ...
        settags = get_tags(instance_id=instance_id, call="action", location=location)
...
def get_tags(
    name=None, instance_id=None, call=None, location=None, kwargs=None, resource_id=None
):  # pylint: disable=W0613
    """
...
def del_tags(
    name=None, kwargs=None, call=None, instance_id=None, resource_id=None
):  # pylint: disable=W0613
    """
...
def rename(name, kwargs, call=None):
    ...
    salt.utils.cloud.rename_key(__opts__["pki_dir"], name, kwargs["newname"])
...
def destroy(name, call=None):
    ...
    if protected == "true":
        raise SaltCloudSystemExit(
            ...
            "salt-cloud -a disable_term_protect {}".format(name)
        )
    ...
    rename_on_destroy = config.get_cloud_config_value(
...
def reboot(name, call=None):
    ...
    """
    instance_id = _get_node(name)["instanceId"]
    params = {"Action": "RebootInstances", "InstanceId.1": instance_id}
...
def show_detailed_monitoring(name=None, instance_id=None, call=None, quiet=False):
    ...
    log.log(
        logging.DEBUG if quiet is True else logging.INFO,
        "Detailed Monitoring is %s for %s",
        matched["monitoring"],
        name,
    )
...
def disable_detailed_monitoring(name, call=None):
    ...
    instance_id = _get_node(name)["instanceId"]
    params = {"Action": "UnmonitorInstances", "InstanceId.1": instance_id}
...
def enable_detailed_monitoring(name, call=None):
    ...
    instance_id = _get_node(name)["instanceId"]
    params = {"Action": "MonitorInstances", "InstanceId.1": instance_id}
...
def _toggle_delvol(
    ...
    params = {"Action": "ModifyInstanceAttribute", "InstanceId": instance_id}
    ...
    kwargs = {"instance_id": instance_id, "device": device, "volume_id": volume_id}
...
def create_volume(kwargs=None, call=None, wait_to_finish=False):
    ...
    if "tags" in kwargs:
        ...
        if isinstance(tags, dict):
            new_tags = set_tags(
                tags=tags, resource_id=volume_id, call="action", location=get_location()
            )
            r_data["tags"] = new_tags

    # Waits till volume is available
    if wait_to_finish:
        salt.utils.cloud.run_func_until_ret_arg(
            fun=describe_volumes,
...
def detach_volume(name=None, kwargs=None, instance_id=None, call=None):
    ...
    params = {"Action": "DetachVolume", "VolumeId": kwargs["volume_id"]}
...
def delete_volume(name=None, kwargs=None, instance_id=None, call=None):
    ...
    params = {"Action": "DeleteVolume", "VolumeId": kwargs["volume_id"]}
...
def import_keypair(kwargs=None, call=None):
    ...
    """
    if call != "function":
        log.error("The import_keypair function must be called with -f or --function.")
        return False
...
def show_keypair(kwargs=None, call=None):
    ...
    params = {"Action": "DescribeKeyPairs", "KeyName.1": kwargs["keyname"]}
...
def delete_keypair(kwargs=None, call=None):
    ...
    """
    if call != "function":
        log.error("The delete_keypair function must be called with -f or --function.")
        return False
    ...
    params = {"Action": "DeleteKeyPair", "KeyName": kwargs["keyname"]}
...
def create_snapshot(kwargs=None, call=None, wait_to_finish=False):
    ...
    if volume_id is None:
        raise SaltCloudSystemExit("A volume_id must be specified to create a snapshot.")
...
def get_console_output(
    ...
    params = {"Action": "GetConsoleOutput", "InstanceId": instance_id}
    ...
    for item in data:
        if next(iter(item.keys())) == "output":
            ret["output_decoded"] = binascii.a2b_base64(next(iter(item.values())))
        else:
